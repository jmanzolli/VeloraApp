...
st.set_page_config(
    page_title="Bike Network Planner",
    layout="wide",
...
st.markdown(
    ...
    <div class="topbar">
      ...
      <div class="topbar-title">
        <div class="brand-lockup">
          <img class="brand-logo" src="__VELORA_BADGE__" alt="Bike Network Planner logo" />
          <span>Bike Network Planner</span>
        </div>
      </div>
    ...
    <div class="hero">
      <h1>Plan better cycling networks</h1>
...
with workflow_cols[0]:
    st.markdown("**Complete Data Planner**")
    st.caption("Use existing station demand and LTS network files, or load the built-in demo scenario.")
    if st.button("Load complete sample", use_container_width=True):
        st.session_state["active_run_id"] = DEMO_RUN_ID
        st.rerun()
with workflow_cols[1]:
    st.markdown("**No Station Locations**")
    st.caption("Create artificial station candidates from population polygons, then optimize them.")
    if st.button("Build artificial stations", use_container_width=True):
        st.switch_page("pages/Population_Demand_Builder.py")
with workflow_cols[2]:
    st.markdown("**No Lanes / LTS Network**")
    st.caption("Generate a starter LTS network from OSM and optional municipal layers.")
    if st.button("Build LTS network", use_container_width=True):
...
def selected_artifact(artifacts: list[dict[str, Any]], selected_label: str | None) -> dict[str, Any] | None:
    if not selected_label:
        return None
    return next((artifact for artifact in artifacts if artifact_label(artifact) == selected_label), None)


def artifact_path(artifact: dict[str, Any] | None) -> Path | None:
    if artifact is None:
        return None
...
def render_sidebar_artifact_status(artifact: dict[str, Any] | None, kind_label: str) -> None:
    ...
    st.sidebar.markdown(
        ...
        <div class="sidebar-upload-status">
          <div class="sidebar-upload-icon">✓</div>
          <div class="sidebar-upload-text">
            <div class="sidebar-upload-name">{html.escape(str(artifact.get("label", artifact.get("artifact_id", "Saved input"))))}</div>
            <div class="sidebar-upload-meta">{kind_label} saved · {int(artifact.get("row_count") or 0):,} rows</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.sidebar.markdown(
        ...
        <div class="sidebar-brand-card">
          <div class="sidebar-brand-top">
            <img class="sidebar-brand-logo" src="{BADGE_URI}" alt="Bike Network Planner badge" />
            <div>
              <p class="sidebar-kicker">Urban Cycling Studio</p>
              <p class="sidebar-wordmark">Bike Network Planner</p>
            </div>
          </div>
...
def generate_report_markdown(run_payload: dict[str, Any]) -> str:
    ...
    best_effort = solutions.get("Best Effort", {}).get("metrics", {})
    ...
    lines = [
        ...
        (
            ...
            f"Modeled cyclist effort is **{recommended.get('effort', 0.0):,.1f}**. "
    ...
    if best_stress or best_cost or best_demand or best_effort:
        ...
        if best_effort:
            impact = estimate_impact_metrics(float(best_effort["demand"]), config)
            lines.append(
                f"- **Best Effort**: {best_effort['demand']:,.0f} trips, effort {best_effort.get('effort', 0.0):,.1f}, cost {best_effort['cost']:,.0f}, emissions reduction {format_emissions(impact['Emissions Reduction'])}."
            )
        if best_demand:
...
with st.sidebar.expander("Open data setup", expanded=True):
    st.markdown(
        '<p class="upload-section-copy">Use complete uploads or saved/generated inputs from the artificial station and LTS builders.</p>',
        unsafe_allow_html=True,
    )
    demand_artifacts = list_demand_artifacts()
    lts_artifacts = list_lts_artifacts()

    station_source = st.radio(
        "Station input source",
        ["Upload station file", "Use saved/generated artificial stations"],
        horizontal=False,
    )
    stations_upload = None
    selected_demand_artifact = None
    if station_source == "Upload station file":
    ...
    elif demand_artifacts:
        demand_labels = [artifact_label(artifact) for artifact in demand_artifacts]
        preferred = st.session_state.get("selected_demand_artifact_id")
        preferred_index = next(
            (idx for idx, artifact in enumerate(demand_artifacts) if artifact.get("artifact_id") == preferred),
            0,
        )
        selected_demand_label = st.selectbox("Saved artificial stations", demand_labels, index=preferred_index)
        selected_demand_artifact = selected_artifact(demand_artifacts, selected_demand_label)
        render_sidebar_artifact_status(selected_demand_artifact, "Artificial stations")
    else:
        st.warning("No saved artificial station networks yet.")
        if st.button("Create artificial stations", use_container_width=True):
            st.switch_page("pages/Population_Demand_Builder.py")
    ...
    if network_source == "Upload LTS network":
        ...
        render_sidebar_artifact_status(selected_lts_artifact, "LTS network")
    else:
        st.warning("No saved LTS networks yet.")
        if st.button("Create LTS network", use_container_width=True):
            st.switch_page("pages/LTS_Calculator.py")

station_input = artifact_path(selected_demand_artifact) if selected_demand_artifact else stations_upload
...
def save_uploaded_temp_file(uploaded_file) -> tuple[str, bool]:
    if isinstance(uploaded_file, (str, Path)):
        return str(uploaded_file), False
...
def build_run_payload(artifacts, config: UIConfig, run_id: str) -> dict[str, Any]:
    ...
    for solution_name, solution in artifacts.solutions.items():
        ...
        for edge in solution["selected_links"]:
            edge_key = (repr(edge[0]), repr(edge[1]))
            ...
            "selected_links": [
                {
                    ...
                    "mean_effort": float(row["mean_effort"]) if pd.notna(row.get("mean_effort")) else None,
                    "effort_exposure": float(row["effort_exposure"]) if pd.notna(row.get("effort_exposure")) else None,
                    "max_steepness_level": float(row["max_steepness_level"]) if pd.notna(row.get("max_steepness_level")) else None,
                }
                for row in selected_link_rows
            ],
        }
...
def run_local_optimization(station_file, network_file, form_data: dict[str, Any]) -> dict[str, Any]:
    ...
    status_box = st.empty()
    run_id = new_run_id()
    stations_path, cleanup_stations = save_uploaded_temp_file(station_file)
    network_path, cleanup_network = save_uploaded_temp_file(network_file)
        ...
        rows.append(
            {
                ...
                "Average Effort": metrics["Average Effort"],
...
def get_solution_display_metrics(solution: dict[str, Any], config: dict[str, Any] | None = None) -> dict[str, float]:
    ...
    avg_effort = metrics.get("avg_effort")
    if avg_effort is None:
        avg_effort = float(metrics.get("effort", 0.0)) / max(1, int(metrics.get("selected_links", 0)))
    ...
    return {
        ...
        "Average Effort": float(avg_effort),
...
def build_difference_summary(candidate_entry: dict[str, Any], baseline_entry: dict[str, Any]) -> dict[str, Any]:
    ...
    def link_keys(df: pd.DataFrame) -> set[tuple[str, str]]:
        ...
        return {(from_name, to_name) for from_name, to_name in zip(from_series, to_series)}
...
def render_planning_metric_grid(metrics: dict[str, Any], baseline: dict[str, Any] | None = None) -> None:
    metric_specs = [
        ...
        ("Average effort", f"{metrics['Average Effort']:.2f}", "lower effort is better", "Average Effort", True, True),
    ]
    cards = []
    for label, value, note, key, lower_is_better, wide in metric_specs:
        ...
        if baseline:
            delta_text = (
                ...
                if key in {"Average LTS", "Average Effort"}
...
def render_comparison_delta_cards(candidate_metrics: dict[str, Any], baseline_metrics: dict[str, Any]) -> None:
    specs = [
        ...
        ("Average effort", candidate_metrics["Average Effort"], baseline_metrics["Average Effort"], True, "cyclist effort"),
    ]
    cards = []
...
if station_input is None or network_input is None:
    st.info("Choose or create a station input and an LTS network input to begin.")
else:
    if stations_upload is not None:
        render_station_preview(stations_upload)
    else:
        st.success("Saved/generated station input selected.")
    if selected_lts_artifact is not None:
        st.success("Saved/generated LTS network selected.")

if run_button:
    if station_input is None or network_input is None:
        st.error("Please choose or create both station and LTS network inputs first.")
        st.stop()
