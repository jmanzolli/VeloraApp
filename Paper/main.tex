...
This paper presents \textit{Velora} (Visualization and Evaluation of Low-stress Optimization for Bicycle Resource Allocation), a decision-support platform for integrated planning of bike-share systems and cycling infrastructure. Velora operationalizes a multi-objective optimization workflow that jointly determines station locations, bikeway upgrades, and station capacity while balancing three competing planning objectives: maximizing demand coverage, minimizing cyclist exposure to traffic stress, and controlling procurement costs. To align infrastructure design with transport-environment appraisal, the platform also formalizes modal-shift and avoided-emissions indicators that estimate potential vehicle-kilometres and CO$_2$ reductions under transparent substitution assumptions. The platform links data ingestion, candidate generation, level-of-traffic-stress mapping, NSGA-II optimization, scenario storage, interactive interpretation, and automatic report generation into a reproducible workflow for planners and policy analysts. Applied to Montreal using origin-destination demand from BIXI trip records and GIS-derived level-of-traffic-stress classification, Velora generates Pareto-efficient scenarios that can be compared through maps, metrics, emissions-potential indicators, and policy-oriented summaries. Results demonstrate that substantial demand (4.6 million trips annually) can be served through selective investment in 93 stations and 72 links at CAD\$ 9.42 million, compared with demand-maximizing scenarios that require nearly five times as much. Under an illustrative assumption that 5\% of served bike-share trips replace car trips of 2.0~km, the balanced solution avoids approximately 461,300 vehicle-km and 115 tonnes of tailpipe CO$_2$ annually. These findings indicate that platform-based planning can help agencies move from static network evaluation toward transparent scenario testing, allowing limited infrastructure budgets to be allocated among coverage, comfort, cost, and climate-mitigation objectives in a defensible manner.
...
The paper is organized as follows. Section~\ref{review} reviews literature on cycling accessibility and infrastructure planning. Section~\ref{method} presents the methodological approach, including data acquisition, platform architecture, and model formulation. Section~\ref{casestudy} describes the Montreal case study. Section~\ref{results} discusses the results, including Pareto-optimal outcomes and sensitivity analyses. Section~\ref{policy} presents policy-making directions stemming from the results of the optimization experiments and their implementation in Velora. Section~\ref{conclusion} concludes with key takeaways, limitations, and directions for future research.
