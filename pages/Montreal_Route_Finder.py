...
def montreal_network_path() -> Path:
    ...
    candidates.extend(sorted(GENERATED_NETWORK_DIR.glob("*.geojson")) if GENERATED_NETWORK_DIR.exists() else [])
    ...
    for path in candidates:
        ...
        if "effort_exposure" in network and network["effort_exposure"].notna().any():
            prepared.append((len(network), path))
    if prepared:
        return max(prepared, key=lambda item: item[0])[1]
    raise FileNotFoundError(
        "No prepared Montreal network with cyclist effort is available. "
        "Create and save one in LTS Network Builder before using Route Finder."
    )
